...
let
  ...
  cfg = config._.givon.wayland.sway.mako;
in
{
  options._.givon.wayland.sway.mako.enable = mkEnableOption "mako";
  config.home-manager.users.${name} = mkIf cfg.enable {
    services.mako =
      let
